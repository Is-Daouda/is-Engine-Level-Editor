            ...
            WITH(m_gridCaseList.size())
            {
                ...
                if (is::instanceExist(*it))
                {
                        ...
                        static_cast<int>((*it)->getY()) == static_cast<int>(m_cursor.getY()))
                    {
                        ...
                        {
                            std::shared_ptr<Tile> t = std::shared_ptr<Tile>(new Tile(setCorrespondingTex(m_cursor.m_tileNum),
...
void LevelEditor::createGridCase()
{
    for (unsigned int i = 0; i < m_levelHeight; i++)
    {
        ...
        {
            m_gridCaseList.push_back(std::shared_ptr<GridCase>(new GridCase(j * 32.f, i * 32.f)));
        }
    }
}
    ...
    if (!autoSaveCall)
    {
        if (m_levelPath == "")
        {
            m_levelPath = is::TinyDialogBox::showFileDialogBox(is::TinyDialogBox::FileDialogType::SAVE_FILE,
                                                                 ...
                                                                 is::strToWStr(m_levelName).c_str(),
                                                                 L"Can't save file!");
        }
    ...
    if (file)
    {
        ...
        WITH(m_gridCaseList.size())
        {
            for (unsigned int i(0); i < is::arraySize(m_gridCaseList[_I]->m_tileNumArray); i++)
            {
                ...
                {
                    // insert tile
                    std::shared_ptr<Tile> t = std::shared_ptr<Tile>(new Tile(setCorrespondingTex(m_gridCaseList[_I]->m_tileNumArray[i]),
...
bool LevelEditor::saveMapAsCHeader()
{
    ...
    wchar_t const *lFilterPatterns[2] = {L"*.h", L"*.header"};
    headerFilePath = is::TinyDialogBox::showFileDialogBox(is::TinyDialogBox::FileDialogType::SAVE_FILE,
                                                          L"Save Level As C Header",
                                                          lFilterPatterns,
                                                          is::strToWStr(m_levelName).c_str(),
                                                          L"Can't save file as C Header!");
